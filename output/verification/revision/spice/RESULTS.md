...
| measurement | value | expected | result |
|---|---|---|---|
| coil current while BLANKING is high (reset, +5V ramp, undefined latch content = 1) (A): the tri-stated 74HCT574 and the 10 k gate pull-down keep the IPD90N10S4L06 off (`icoil_blank`) | 2.324e-08 | 0 … 0.01 | PASS |
...
| solenoid gate voltage while blanked (V) (`vgate_blank`) | 0.0001309 | 0 … 0.5 | PASS |
...
| coil current after the CPU writes 0 (A) (`icoil_after`) | 2.529e-09 | 0 … 0.01 | PASS |
| column current after the CPU writes 0 (A) (`icol_after`) | 3.15e-05 | 0 … 0.01 | PASS |
...
| gate after the CPU writes bit = 0 (ribbon HIGH) (V) - OFF (`g_w0`) | 0.08671 | 0 … 0.3 | PASS |
| drain at the 70 V rail with the output OFF (V) (`out_w0`) | 71.12 | 60 … 80 | PASS |
| gate after a second write of 1 (V) - ON again (`g_w1b`) | 4.478 | 4 … 5 | PASS |
...
| 74HCT574 output current with the ribbon unplugged (A): SR2 pulls the data HIGH (bit 0), SR1 holds the strobes HIGH (no clock) and BLANKING HIGH -> every latch tri-stated (/OE) - the 10 k pull-down then discharges the gate in ~150 us (powerup.cir) (`ioe_unplug`) | 7.114e-10 | 0 … 1e-06 | PASS |
...
| gate after the final write of 0 (V) (`g_w0b`) | 0.07212 | 0 … 0.3 | PASS |
