...
class Board:
        ...
        for f in find_all(self.doc,'footprint'):
            ...
            for p in find_all(f,'pad'):
                ...
                self.items.append(dict(id=uid(p),net=net(p),poly=poly,layers=layers,pin=f'{ref}.{p[1]}',hole_clearance=.25 if p[2]=='np_thru_hole' else 0,
                                       local_clearance=float(override[1]) if override else None))
                dr=find(p,'drill')
                if dr:
    ...
    def route(self,n,pitch,grid_margin=None,ground_pin=None,local=True,source_pin=None):
        ...
        options=getattr(self,'routing_options',{})
        width=options.get('width',width);dia=options.get('via_diameter',dia);drill=options.get('via_drill',drill)
        if ground_pin:
            ...
            width=options.get('width',width);clear=options.get('clearance',clear)
            dia=options.get('via_diameter',dia);drill=options.get('via_drill',drill)
        ...
        if width<=.3 and not options:dia,drill=.52,.25
        if local:
        ...
        obs=[unary_union([v['poly'].buffer(max(clear,v.get('local_clearance') if v.get('local_clearance') is not None else self.params(v['net'])['clearance'],v.get('hole_clearance',0))+.008) for v in foreign if l in v['layers']]) for l in range(3)]
        for l in (0,2):
