...
p=argparse.ArgumentParser();p.add_argument('trace',type=Path);p.add_argument('--rail',type=float,default=20.69);p.add_argument('--max-step-us',type=float,default=2);p.add_argument('--strict',action='store_true');p.add_argument('--output',type=Path,default=ROOT/'output/verification/prototype-readiness/replay-matrix');a=p.parse_args()
...
# Keep the full circuit and 20 ns stimulus edges. A 1 ps minimum breakpoint
# separation avoids the failing default near-coincident breakpoint handling in
# this Gear configuration. It is 20,000 times smaller than the shortest edge.
# Full 1/0.5 us and cross-method comparisons are retained in prototype-readiness.
source+=f'''.options method=gear reltol={'.0001 trtol=1 chgtol=1e-16' if a.strict else '.002'} klu itl4=500 minbreak=1p
...
source+='.endc\n.end\n';name=f'matrix_{a.rail:g}V'+(f'_{a.max_step_us:g}us' if a.max_step_us!=2 else '')+('_strict' if a.strict else '');path=out/(name+'.cir')
if path.exists():raise FileExistsError(f'Preserve previous simulation before rerunning: {path}')
