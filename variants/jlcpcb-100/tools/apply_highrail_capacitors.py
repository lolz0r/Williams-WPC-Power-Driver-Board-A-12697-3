...
assert 'C64' not in fps,'One-shot migration already applied'
code='C3445915';cat=json.loads((ROOT/'research/jlcpcb/catalog'/(code+'.json')).read_text());name='CP_Radial_D18.0mm_P7.50mm';fpname='Capacitor_THT:'+name
props={'Value':'1000uF 100V','MPN':cat['componentModelEn'],'Manufacturer':cat['componentBrandEn'],'JLCPCB Part':code,'JLCPCB Library':'Extended','Link':cat['url'],'Footprint':fpname,'Description':'Samyoung NHA 1000uF100V, 2.02A at120Hz105C, D18x40mm P7.5; four-piece 4000uF high-rail bank.','Price':'','Price100':''}
mapping={'C8':'C64','C32':'C65'};uuids={r:str(uuid.uuid4()) for r in mapping.values()};nets={r:{p.GetNumber():p.GetNetname() for p in fps[r].Pads()} for r in mapping}
def field(sym,k,v):
...
for idx,(oldref,newref) in enumerate(mapping.items()):
 ...
 sym=copy.deepcopy(orig);ax,ay=map(float,find(sym,'at')[1:3]);x,y=300.99+idx*27.94,400.0;find(sym,'at')[1:]=[x,y,0];find(sym,'uuid')[1]=uuids[newref]
 ...
 for pin,dy in [('1',-3.81),('2',3.81)]:
  ...
  sch.append(node)
 sch.append(S('text',f'{newref} parallels {oldref}: pair 2000uF; high rail total 4000uF',S('at',x,y+10.16,0),S('effects',S('font',S('size',.9,.9)),S('justify',Sym('left'))),S('uuid',str(uuid.uuid4()))))
...
for oldref,newref in mapping.items():
 ...
 horizontal=True;centers=[(324,93),(344,93)] if oldref=='C8' else [(284,93),(304,93)];cx=(centers[0][0]+centers[1][0])/2;cy=93
...
for ref,x,y in [('R196',352,79),('LED6',352,84)]:fps[ref].SetPosition(vec(x,y))
pcb.SaveBoard(str(ROOT/'wpc_power_driver_cost.kicad_pcb'),b)
(ROOT/'research/selection.json').write_text(json.dumps(plan,indent=2)+'\n');(ROOT/'research/highrail-capacitor-migration.json').write_text(json.dumps(record,indent=2)+'\n')
print('Installed four individually annotated 1000uF100V NHA capacitors')
