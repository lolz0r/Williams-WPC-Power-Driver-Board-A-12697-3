...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--gap-aware',action='store_true');p.add_argument('--pitch',type=float,default=2.);p.add_argument('--bridge-dir',type=Path,default=Path('output/verification/prototype-readiness/bridge-corners'));args=p.parse_args()
...
report={'method':f'{h} mm finite-volume, gap-aware={args.gap_aware}, 4 copper sheets, FR4 dielectric coupling and 20 um plated through barrels. D2PAK 1.5 C/W junction-to-pad. TO220 dedicated junction/sink nodes; 8 W distributed background plus simultaneous simulated corner bridge losses and max leakage.',
