"""Datasheet-based averaged supply tests for the JLC-3 TPS552892 circuits.
...
def phase_sweep(rail,rc):
    ...
    for vin,cap_scale,esr,gm_scale,r_scale,cc_scale,cp_scale in itertools.product(
            [6.4,9,15] if five else [8.4,11,15,21], [.8,1.2], [.022,.044], [.8,1.2], [.99,1.01], [.75,1.25], [.95,1.05]):
...
def deck(rail,scenario,rc,step_us,frequency,line,cap_scale=.8,reference_scale=1,bulk_esr=.022,gm_scale=1):
    five=rail['ref']=='U20';vout=1.2*(1+rail['fb_ohm']/(2100 if five else 2000))
    full=3 if five else .75;bottom=2100 if five else 2000;cap=8000e-6 if five else 16000e-6
    ...
    return f'''JLC-3 {rail['ref']} {scenario}; engineering averaged controller, not TI vendor model
...
Bcompclamp comp 0 I=max(v(comp)-1.3,0)-max(.7-v(comp),0)+ternary_fcn(v(enable)<.5,(v(comp)-.7),0)
Rcomp comp cc {rc}
Ccomp cc 0 {rail['cc_f']} IC=.7
Cpole comp 0 {rail['cp_f']} IC=.7
Rgain comp 0 1e9
Bcommand command 0 V=min(max((v(comp)-.7)/.055,0),8)*v(enable)
Rinner command il 1
Cinner il 0 4u
...
.ic v(comp)=.7 v(cc)=.7 v(reg)={pre} v(out)={pre}
...
def main():
    ...
    p.add_argument('--rc5',type=float,default=33200);p.add_argument('--rc12',type=float,default=42200)
    ...
    p.add_argument('--cp12',type=float,default=1e-9);p.add_argument('--cp5',type=float,default=680e-12);p.add_argument('--cap-scale',type=float,default=.8);p.add_argument('--reference-scale',type=float,default=1);p.add_argument('--bulk-esr',type=float,default=.022);p.add_argument('--gm-scale',type=float,default=1)
    ...
    if not a.phase_only:
        ...
        for rail,scenario,frequency,line in itertools.product(rails,['startup','load_step','loss_restart','prebiased'],[50,60],[.9,1.1]):
            ...
            checks=dict(completed=complete,settled=complete and v['settled_min']>=lo and v['settled_max']<=hi,
