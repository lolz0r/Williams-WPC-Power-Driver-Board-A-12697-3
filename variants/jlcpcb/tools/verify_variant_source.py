"""Audit the actual JLC-2 native source against the preserved parent board."""
...
check(set(old)-set(new)=={'SR1','SR2'},'Only the two SIP resistor arrays were removed')
check(set(new)-set(old)=={f'R{i}' for i in range(310,326)},'Exactly sixteen new discrete pull-ups')
...
report=dict(passed=all(c['passed'] for c in checks),checks=len(checks),failures=[c for c in checks if not c['passed']],pcb_sha256=sha(ROOT/'wpc_power_driver_cost.kicad_pcb'),original_board_sha256=sha(BASE/'wpc_power_driver_cost.kicad_pcb'),preserved_baseline_files=expected,scope='Existing component positions and pad geometry unchanged. SIP arrays replaced by sixteen correctly connected resistors. JLC-2 explicitly merges SOL21..28_TB into +20V for STTNG flashers; other pad nets retained. Source consistency and model-input hashes; not a complete transistor-level extraction of this PCB.')
