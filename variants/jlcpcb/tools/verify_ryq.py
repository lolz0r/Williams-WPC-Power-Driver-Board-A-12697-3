...
for fp in find_all(b,'footprint'):
 ...
 check(angle==0 and props['MPN']=='TPS552892RYQR',ref+' selected package and orientation')
...
report=dict(passed=all(c['passed'] for c in checks),checks=len(checks),failures=[c for c in checks if not c['passed']],pcb_sha256=hashlib.sha256(pcb.read_bytes()).hexdigest(),source='TI RYQ0021B 4228792/A, recommended land and stencil; TPS552892 SLVSH28A pin table',limitations='Checks native package geometry and orientation; JLCPCB library rotation mapping must be checked in the actual assembly preview')
