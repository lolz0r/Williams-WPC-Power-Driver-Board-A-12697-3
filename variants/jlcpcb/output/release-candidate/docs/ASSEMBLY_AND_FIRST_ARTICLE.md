> JLC-3 retains this mechanical and connector scheme. Use the current [prototype report](PROTOTYPE_JLC3.md) and matching `output/release-candidate` files. Physical first-article qualification follows fabrication.
...
| Supply bring-up | Use isolated, current-limited secondary sources and dummy loads, beginning with the raw-input paths. Observe +5 V, +12 V, raw rails, switch nodes and input/output currents. Check prebiased-output and power-loss behavior first with current-limited dummy-load fixtures; the new converters use shutdown load disconnect and PFM operation. Switching/backfeed currents have not been measured on hardware. |
...
[TI TPS552892](https://www.ti.com/lit/ds/symlink/tps552892.pdf),
