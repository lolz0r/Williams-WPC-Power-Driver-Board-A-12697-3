...
parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--reuse',action='store_true',help='Reuse a log only when its exact generated input deck matches');args=parser.parse_args()
...
minimum_lib.write_text(original.replace('-1.22u,61.2u)',f'-1.22u,{4.5/91400:.12g})'),encoding='latin1')
...
for rail,vin,minimum in cases:
 ...
 name=f'{rail}_vin{vin}'+('_minimum_limit' if minimum else '');p=OUT/(name+'.cir')
