...
for frequency,line,esr,imbalance in itertools.product((50,60),(1.,1.1),(.02,.05,.1),(False,True)):
 ...
 checks={k:dict(s,measured_A=values.get(k),passed=k in values and values[k]<=s['rating_A']) for k,s in limits.items()}
 results.append(dict(name=name,frequency_Hz=frequency,line_scale=line,checks=checks,
                     passed=all(c['passed'] for c in checks.values() if c['load']=='design load')))
 print(name, 'PASS' if results[-1]['passed'] else 'FAIL',flush=True)
report=dict(cases=results,passed=all(r['passed'] for r in results),
