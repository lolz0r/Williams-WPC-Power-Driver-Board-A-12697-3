...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('trace',type=Path);p.add_argument('--output',type=Path,required=True);p.add_argument('--channels',nargs='+',type=int,choices=range(1,17),default=list(range(1,17)));p.add_argument('--max-step-us',type=float,default=5.);a=p.parse_args();assert 0<a.max_step_us<=100
...
.save v(drainq) v(gate) i(Vsense) i(Vdiode) i(Lcoil)
...
* Average forward current; exclude physical reverse leakage from the forward rating.
let id=(abs(i(Vdiode))+i(Vdiode))/2
...
'''
  ...
  result=dict(deck_sha256=hashlib.sha256(source.encode()).hexdigest(),channel=channel,window_s=[start,end],commanded_duty=duty,coil_ohm=resistance,coil_H=inductance,rail_V=86,checks=checks,errors=errors,passed=not errors and all(c['passed'] for c in checks.values()))
  results.append(result);print(channel,'PASS' if result['passed'] else 'FAIL',values,flush=True)
 report=dict(max_step_us=a.max_step_us,requested_channels=a.channels,trace_sha256=hashlib.sha256(a.trace.read_bytes()).hexdigest(),model_sha256=hashlib.sha256((HERE/'models.lib').read_bytes()).hexdigest(),cases=results,passed=bool(results) and all(r['passed'] for r in results),limitations=['Trapezoidal integration, maximum step recorded in report, 20 ns command transitions, strict PWL times and explicit end-time completion; no partial transient may pass','Coil R/L approximations; 86 V high-line bound with 4.5 V gate and hot MOSFET fit','Commands assumed enabled; no credit for physical blanking','Temperature is steady repetition of worst 10 s window at 50 C ambient and 62 C/W, excludes mutual heating','Channels 17-28 motors/flashers excluded here; covered by generic decks only','No harness parasitics, avalanche SOA, fuse clearing or physical thermal correlation'])
