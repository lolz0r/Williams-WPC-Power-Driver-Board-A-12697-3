...
def main():
    ...
    command = ['mame', 'sttng_l7', '-rompath', str(args.rompath), '-pluginspath',
               str(ROOT / 'tools/mame/plugins') + ';/usr/local/share/mame/plugins',
               '-plugin', 'wpcverify', '-video', 'none', '-sound', 'none', '-nothrottle',
    ...
    metadata = {'command': command, 'rom_sha256': hashes, 'scenario': args.scenario,
